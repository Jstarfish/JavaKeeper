...
同步指任务按照调用顺序依次执行，每个任务必须等待上一个任务完成后才能开始。调用方会阻塞，直到任务完成后才能继续处理其他工作。

**特点**

- **阻塞式**：调用方必须等待任务完成。
- **流程简单**：执行顺序直观，便于调试和管理。
- **性能可能受限**：因为一个任务的执行会阻止其他任务的进行。

异步指任务可以在后台执行，调用方无需等待任务完成，可以直接继续处理其他工作。当异步任务完成后，通常会通过回调函数、消息通知或事件来处理结果。

#### **特点**

- **非阻塞式**：调用方无需等待任务完成，可以继续执行后续代码。
- **提高效率**：多个任务可以并发执行，充分利用资源。
- **复杂性增加**：需要额外管理回调或结果的同步。
...
**上下文切换**是指计算机在多任务执行时，将一个任务的运行状态保存，并切换到另一个任务的过程。任务可以是**进程**或**线程**。上下文切换的目的是实现多任务系统，能够同时管理多个任务，即使 CPU 一次只能执行一个任务。

> 线程的上下文切换是上下文切换的一种特殊情况，特指在同一进程内不同线程之间的切换。
>
...
在 Java 中，`synchronized` 是一种常用的同步机制，用于确保多线程环境下对共享资源的访问是线程安全的。其底层实现基于 JVM 和操作系统的支持。

其应用层的语义是可以把任何一个非null对象作为"锁"，

**同步代码块**：`synchronized (lock) { ... }` 需要指定一个对象作为锁。

**同步方法**：`public synchronized void method() { ... }` 使用方法所在对象实例（非静态）或类对象（静态）作为锁。

**作用**：确保线程在进入同步块或同步方法前，必须先获得锁
...
`Monitor` 是同步的核心实现，在 JVM 中由操作系统的互斥量（Mutex）支持。

- 每个对象在同步时关联一个 `Monitor`。
-  包含：
  - **Owner**：记录当前持有锁的线程。
  - **Entry List**：等待获取锁的线程列表。
  - **Wait Set**：调用 `wait()` 方法的线程列表。
...
每个对象在 JVM 中都有对象头（Object Header），其中包含：

1. **Mark Word**：用于存储对象的运行时状态，包括锁状态、HashCode、GC 信息等。
...
- **轻量级锁（Lightweight Lock）**：存储指向锁记录的指针。
- **重量级锁（Heavyweight Lock）**：存储指向 Monitor 对象的指针。

在 JDK 1.6 后，JVM 为了提高锁的获取与释放效率对（synchronized ）进行了优化，引入了 偏向锁 和 轻量级锁 ，从此以后锁的状态就有了四种（无锁、偏向锁、轻量级锁、重量级锁）

JVM 会根据竞争情况优化锁的实现，通过 **偏向锁** 和 **轻量级锁** 减少开销。
...
如果线程A继续再次获得这个锁呢?比如一个方法是synchronized,递归调用自己,那么第一次已经获得了锁,第二次调用的时候还能进入吗? 直观上当然需要能进入.这就要求必须是可重入的，不然就死锁了。 
