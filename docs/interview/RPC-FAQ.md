...
### gRPC 是什么？和传统 RPC 框架（如 Dubbo、Thrift）的区别？

gRPC 是 Google 开源的一个高性能、跨语言的 RPC 框架，基于 HTTP/2 协议，默认使用 Protobuf 进行序列化，支持多路复用和流式通信。相比传统 RPC 框架：

- **和 Dubbo** 比，Dubbo 更偏 Java 生态，和 Spring Cloud 结合紧密；gRPC 更跨语言，更适合云原生和多语言场景。
- **和 Thrift** 比，Thrift 也很高效，但比较老，缺少 HTTP/2 和流式通信，社区活跃度不如 gRPC。
   所以在实际使用中，**如果是 Java 内部服务，我可能选 Dubbo；如果是跨语言或云原生环境，我会选 gRPC。**
