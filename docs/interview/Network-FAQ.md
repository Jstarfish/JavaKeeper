...
![img](https://ask.qcloudimg.com/http-save/yehe-5359587/tucwo70kk6.jpeg)
...
![img](https://ask.qcloudimg.com/http-save/yehe-5359587/cs0sawogj4.jpeg)
...
- 第一次挥手(FIN=1，seq=x)
  ...
  主机1（可以使客户端，也可以是服务器端），设置seq=x，向主机2发送一个FIN报文段；此时，主机1进入`FIN_WAIT_1`状态；这表示主机 1 没有数据要发送给主机2了；
...
滑动窗口机制的基本思想是允许发送方在接收到接收方的确认（ACK）之前，发送多个数据包，而不是每发送一个数据包就等待确认。发送窗口（Send Window）和接收窗口（Receive Window）是滑动窗口机制的核心部分。
