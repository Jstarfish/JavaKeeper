...
滑动窗口是 TCP 实现流量控制的关键机制，通过动态调整发送方的发送速率，避免接收方缓冲区溢出。其核心逻辑可拆解为以下维度：

1. **窗口的本质：缓冲区的可视化抽象**

- **发送方窗口**：维护「已发送未确认」和「可发送」的数据范围，由接收方告知的窗口大小（Window Size）决定。
- **接收方窗口**：接收方缓冲区的剩余空间，通过 ACK 包中的 Window 字段通知发送方（单位：字节）。
...
TCP 滑动窗口通过接收方告知的窗口大小（Window）限制发送方的发送量：

1. 接收方根据缓冲区剩余空间动态调整 Window，并通过 ACK 包通知发送方；
2. 发送方维护窗口边界，仅发送窗口内的数据，避免接收方缓冲区溢出；
3. 窗口随 ACK 确认不断滑动，确保数据有序传输且不积压。
