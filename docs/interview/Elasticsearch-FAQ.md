...
> **Elasticsearch 核心概念**
>
> 1. **索引（Index）**：一个索引是一个文档的集合，用于存储和管理相关的文档。每个索引都有自己的名称，用于通过 HTTP REST API 进行引用。
>
> 2. **文档（Document）**：文档是 Elasticsearch 中存储的基本单位。它是一个 JSON 对象，包含字段和值。文档被存储在一个索引中。
>
> 3. **字段（Field）**：每个文档由多个字段组成，字段是文档的最小单元，每个字段都有一个名称和值。
>
> 4. **类型（Type）**：类型用于区分同一索引中的不同文档集合，但在 Elasticsearch 7.x 版本后，已经弃用类型，推荐使用“只有一个类型”的索引设计。
>
> 5. **节点（Node）**：Elasticsearch 集群中的单个实例称为节点。节点可以有不同的角色，如主节点、数据节点和协调节点。
>
> 6. **集群（Cluster）**：Elasticsearch 集群是由多个节点组成的，集群中的节点通过网络互相连接。集群的名称是每个节点的一个标识符。
>
> 7. **分片（Shard）**：索引的数据在 Elasticsearch 中被分成多个分片进行分布式存储。每个分片是一个 Lucene 实例，负责数据的存储和查询。
>
> 8. **副本（Replica）**：副本是分片的复制副本，用于提高查询性能和容错性。如果主分片丢失，可以通过副本来恢复数据。
>
> 9. **倒排索引（Inverted Index）**：Elasticsearch 使用倒排索引来加速文本检索。倒排索引的基本原理是将文档中的每个单词与其出现的文档建立关联，以便快速查找。
>
> 10. **映射（Mapping）**：映射是对 Elasticsearch 文档中字段的定义，定义了字段的名称、类型、分词器等属性。映射类似于数据库中的表结构。
>
> 11. **分析器（Analyzer）**：分析器用于在索引文档时对文本进行分词、去除停用词等处理。分析器包括字符过滤器、分词器和词汇过滤器。
>
